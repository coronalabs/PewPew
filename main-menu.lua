...
local function updateMenuSelection()
	...
	local inactivePlayfillColor = { (55/255)-(0.15), (68/255)-(0.15), (77/255)-(0.15), 1 }
	...
	for i=1,buttonGroup.numChildren do
		...
		if i == focusIndex then
			...
			if ( child.id == "game" ) then
				if next( composer.getVariable("controls") ) == nil then
					child:setFillColor( unpack( inactivePlayfillColor ) )
...
local function updateControlsTable( device, status, deviceName )
	...
	if device and status then
		...
		if not isDeviceValid( device ) then
			return
		end
	...
	-- Else, populate controls table with all current controllers
		...
		for i = 1,#inputDevices do
			...
			if controls[deviceId] == nil and isDeviceValid( device ) then
				controllersTableView:insertRow(
				{
					isCategory = false,
					rowHeight = 28,
...
local function onInputDeviceStatusChanged( event )
	...
	if event.connectionStateChanged and event.device and isDeviceValid( event.device ) then
...
local function onAxisEvent( event )
	...
	if not isDeviceValid( event.device ) then
		return
	end
